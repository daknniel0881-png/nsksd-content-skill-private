...
V10.4 上线后，曲率立即挑战"是不是改得太严格了？使用数字没问题啊，大额数字也完全没关系，只要能验证就直接用，不要一刀切"。复盘真实场景（丁香医生 / 人民日报健康版）：

- 中文健康科普主流写法是**脚注挂尾**：正文写"平均每天只吃进去 10.4 克 [9]"，文末列参考文献
- V10.4 `SOURCE_IN_PARA_PAT` 要求段内出现**显式**「（来源：XXX）/ DOI / URL / 《刊物》」四选一，**完全不认 `[n]` 脚注和"研究/发表/刊文"等软线索**
- 结果：98% 合规写法会被 `orphan_number` 打 high，门控变成"禁止一切数字"，相当于一刀切

### 修改

**`scripts/fact_auditor.py`**

1. 拆分源模式：
   - `SOURCE_STRONG_PAT` — 原有强模式（`（来源：...）` / DOI / URL / 《刊物》）
   - **新增** `SOURCE_SOFT_PAT` — 软模式识别 `[n]` 脚注（1~3 位）+「XX研究+发现/显示/指出/表明」+「试验/临床/队列/综述/meta 分析/发表于/刊文/公众号/官方/白皮书/指南/声明」
2. **规则 A（orphan_number）放宽**：段内命中 strong **或** soft 任一即通过；只有"段内完全没有任何出处语言"的裸奔数字才记 high
3. **规则 D（institution_without_doi）扩展合规载体**：`DOI_URL_JOURNAL_PAT` 加入 `[n]` / 发表于 / 刊文 / 公众号 / 官方声明 / 白皮书 / 指南
4. Docstring 标注"V10.4.1 放宽修复"+ 3 条 fix notes

### 验证

7 用例回归测试（`/tmp/test_v1041.md`）结果：

| 用例 | 内容特征 | 期望 | 实际 |
|------|---------|------|------|
| 1 · 丁香医生膳食纤维 | `25～30 克 ... 10.4 克 [9]` | 通过 | ✅ 通过 |
| 2 · 丁香医生减重 | `4.87 kg ... 3.87 kg [11]` | 通过 | ✅ 通过 |
| 3 · 人民日报步数 | `每天大约8000步 ... ①` + 机构 | 通过 | ✅ 通过 |
| 4 · 肝癌 % | `13.11% ... 6.66% [1]` | 通过 | ✅ 通过 |
| 5 · 真·裸奔 | `40 mg ... 30%` 无任何出处 | high | ✅ high × 2 |
| 6 · weasel | "研究表明，这个方法非常有效" | high | ✅ high |
| 7 · 机构+《刊物》 | 2026年《柳叶刀-区域...》 | 通过 | ✅ 通过 |

终端输出：`发现 3 条可疑项（high=3, medium=0, low=0）` — 全部是应该被拦截的裸奔/懒惰话术，合规写法 0 误杀。

### 结论

大额数字、精确数据可以放心用，只要带出处线索（脚注数字、刊物名、"研究/发表"等）就通过。V10.4 的"事实硬门控"理念保留，但门槛回归**只杀裸奔**的合理区间。
