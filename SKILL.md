---
name: nsksd-content
description: 日生研NSKSD纳豆激酶自媒体内容工厂Skill（v8.3）。当用户提到日生研、NSKSD、纳豆激酶的公众号选题、文章撰写、内容创作、招商文案、标题优化、大会宣传时，必须使用此Skill。v8.3 单入口 `/nsksd` + 模式持久化（auto/guided）+ 每天 10 点定时推 Step 1 选题卡 + 封面 1 张 + 内文 5-8 张配图。基于 5 个串行子 Agent + guard.py 硬校验门控 + 30 天滚动去重 + 飞书卡片长连接回调的多 Agent 调度架构。
---

# 日生研NSKSD纳豆激酶 · 自媒体内容工厂（v8.3）
