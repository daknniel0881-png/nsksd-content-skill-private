...
artifacts/<SID>/step3-facts.json     ← 逐条占位符查证记录
artifacts/<SID>/step3-digest.txt     ← V10.6.1 新增：公众号摘要（一句话总结，≤54字硬约束）
```

#### 步骤 F.1：公众号摘要 step3-digest.txt（V10.6.1 硬约束）

**为什么要单独写**：不传 digest 公众号会自动截取正文前 54 字，结果就是"复制粘贴感"——开头钩子被拦腰截断，读者看到的是不通顺的半句话。必须由 article-writer 主动写一句话总结。

**摘要撰写五条铁律**：

1. **长度硬上限 54 字**（含标点）。微信硬限，超出截断为 53 字 + "…"，必然破句
2. **一句话**：完整一句话（一个句号收尾），不允许两句拼接
3. **结论先行**：第一个分句就是核心判断/数字/结论，不要"今天我们来聊聊"这种引言
4. **禁止照抄正文首段**：必须做信息压缩——抽取全文最关键的 1 个数字 + 1 个结论
5. **不带"本文""今天""下面"等指示词**：摘要会单独显示在订阅号列表，孤立成立

**摘要好坏对照**：

| 类别 | 例子 | 判断 |
|------|------|------|
| ❌ 烂 | "纳豆激酶在动脉粥样硬化中扮演重要角色，今天我们就来聊聊它的科学证据，看看它到底有什么用。" | 25 字凑数说了等于没说 |
| ❌ 烂 | （正文前 54 字直接截）"很多人不知道，体检报告上 LDL-C 在'正常'范围，并不意味着心血管事件就跟你无关。事实上约一半…" | 复制感强，被截在不该截的地方 |
| ✅ 好 | "约 50% 心梗发生在 LDL 正常人群——光降脂不够，还得看 hsCRP 这个炎症指标。"（37 字） | 数字+结论+留钩子 |
| ✅ 好 | "市一医院 97 例临床：纳豆激酶 90 天斑块改善 46.7%，IMT 改善 42.1%。"（35 字） | 机构+人数+时间+核心数字一句搞定 |
| ✅ 好 | "纳豆激酶溶解血栓速度是尿激酶 19 倍，作用 8 小时，临床溶栓药只能撑 20 分钟。"（38 字） | 三个对比数字串成一个反差判断 |

**写完自检**：
- `wc -m step3-digest.txt` 字符数 ≤ 54（中文字符 +标点）
- 摘要是否包含至少 1 个具体数字 / 机构名 / 对比？没有 → 重写
- 把摘要单独读出来，能不能脱离正文成立？不能 → 重写
